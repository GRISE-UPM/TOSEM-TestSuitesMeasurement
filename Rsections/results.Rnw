...
Table~\ref{tab:intermediate-precision-analysis} shows the analysis results. The residual is zero, because as we explained above $s_r = 0$. $s^2_M$ is calculated as \cite[pp. 348-350]{box2005statistics}:
\begin{equation}
...
The results of the analysis is shown in Table~\ref{tab:icc-analysis}. There are strong similarities with Table~\ref{tab:intermediate-precision-analysis} because the models are rather similar. $s_M$ can be calculated from Table~\ref{tab:icc-analysis} as \cite[Table II]{indrayan2013clinical}:
\begin{equation}
\label{eq:icc-component-variance}
s^2_M = \frac{MS(Instrument) - MS(Residual)}{p}
\end{equation}
